...
namespace csvTOxlsx
{
    ...
    public class metodosLista
    {
        ...
        public List<List<object>> leerCSV(string archivoCSV)
        {
            ...
            {
                throw new FormatException ("Error al procesar el CSV. " + ex.Message);
            }
        }
        ...
        public string exportaXLSX(List<List<object>> datos, string plantillaExcel, int fila, int columna, int hoja, string ficheroExcel)
        {
            ...
            using (FileStream file = new(nombreFichero, FileMode.OpenOrCreate, FileAccess.ReadWrite))
            {
                ...
                try
                {
                    ...
                    for (int l = 0; l < datos.Count; l++)
                    {
                        for (int c = 0; c < datos[l].Count; c++)
                        {
                            ...
                            if (contenidoCelda is string) //Primero verificamos si el contenidoCelda es un string
                            {
                                string? contenidoCeldaStr = contenidoCelda as string; //Convertimos el objeto a string para poder buscar la cadena que identifica la formula
                ...
                {
                    throw new FormatException ("No se ha podido transformar el fichero a Excel. Revisar formulas o simbolos extraños" + ex.Message);
                }
                ...
                using (FileStream fileOut = new(ficheroExcel, FileMode.Create))
                {
                    ...
                    {
                        throw new IOException ("No se ha podido guardar fichero Excel. Revisar si esta abierto" + ex.Message);
                    }
                }
            }
